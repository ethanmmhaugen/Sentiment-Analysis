...
The goal of this assignment is to get familiar with how vectors operate. While there are many other data structures which
would allow us to implement a more efficient solution, we will stick with vectors for now. If you want to implement a more efficient version in addition, then you can do so and compare the runtime in your description.
...
* You will be later asked about the time complexity of your implementation. What does the time complexity depend on? The number of tweets? The number of words? The number of entries in one of your data structures? Can you reduce the runtime by knowing this?
...
This assignment is all about vectors, so you need to stick to them for now.
You must implement your own custom string class (`DSString`) as part of this project.  You may not use the STL string class or any other available string class from the Internet.
...
* Week 1: DSString Class and a method to store counters for strings. You should be able
  to represent a set of words with counts. Test with `valgrind` or `asan` to make sure you do not have 
  a memory leak. 
